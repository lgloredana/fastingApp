...
export function RecentHistoryCard({
  ...
  const [sessionToDelete, setSessionToDelete] = useState<FastingSession | null>(
    null
  );

  const handleDeleteClick = (session: FastingSession) => {
  ...
  const handleConfirmDelete = () => {
    if (sessionToDelete) {
      // Start the animation
      setDeletingSessionIds((prev) => new Set([...prev, sessionToDelete.id]));
  ...
  return (
    <>
      ...
      <DeleteConfirmationDialog
        ...
        sessionDate={
          sessionToDelete
            ? format(sessionToDelete.startTime, 'EEEE, dd MMMM yyyy', {
                locale: ro,
              })
            : undefined
        }
        sessionDuration={
          sessionToDelete?.duration
            ? formatTime(sessionToDelete.duration)
            : undefined
        }
      />
    </>
